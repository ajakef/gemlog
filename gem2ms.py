#/usr/bin/python3
# conda create -n g1 python=3.7.6 numpy obspy rpy2
import numpy as np
import sys, os, glob, getopt
import logging
sys.path.append('/home/jake/Dropbox/Gem_logger/gem_package/python/gemlog_python_backup')
import gemlog
import logging
...
def ParseError(e):
    ...
    if(e.find('Unable to allocate') > 0):
        return 'Tried to allocate very large array, probably due to large time gap between adjacent files'
    elif(e.find('NULLType')):
...
def main(argv):
    ...
    test = False
    try:
        #opts, args = getopt.getopt(argv,"hi:s:o:",["inputdir=","serialnumber=","outputdir="])
